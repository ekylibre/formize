...
(($) ->
  ...
  $(document).behave "focusin click keyup change", "input[data-datepicker]", ->
    element = $(this)
    locale = undefined
    options = {}
    name = undefined
    hidden = undefined
    if element.prop("datepickerLoaded") isnt "Yes!"
      locale = element.data("date-locale")
      locale = "en"  if $.datepicker.regional[locale] is null or $.datepicker.regional[locale] is `undefined`
      ...
      hidden.attr "data-observe", "1000"  if hidden.data("observe") is `undefined` or hidden.data("observe") is null  if hidden.data("dependents") isnt `undefined` and hidden.data("dependents") isnt null
