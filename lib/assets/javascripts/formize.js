...
Formize.Overlay = {
    ...
    add: function (body) {
	    ...
	    overlay.attr({id: 'overlay', style: 'position:fixed; top:0; left: 0;'});
            $('body').append(overlay);
	    this.resize();
    	}
        this.count += 1;
    	overlay.css('z-index', this.z_index());
        return overlay;
    },
...
Formize.Dialog = {
    ...
    open: function (url, updated, ratio) {
		...
		var dialog = $(document.createElement('div'));
                dialog.attr({id: dialog_id, 'data-ratio': ratio, 'data-dialog-update': updated, flex: '1', 'class': 'dialog', style: 'z-index:'+(Formize.Overlay.z_index()+1)+'; position:fixed; display: none;'});
                $('body').append(dialog);
                dialog.html(data);
		$
