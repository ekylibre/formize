module YasuiForm
  class FormBuilder < ActionView::Helpers::FormBuilder
    ...
    extend MapType
    include YasuiForm::Inputs

    map_type :text, :file, :to => YasuiForm::Inputs::MappingInput
    map_type :string, :password, :email, :search, :tel, :url, :to => YasuiForm::Inputs::StringInput
    map_type :integer, :decimal, :float, :to => YasuiForm::Inputs::NumericInput
    map_type :select, :radio, :check_boxes, :to => YasuiForm::Inputs::CollectionInput
    map_type :date, :time, :datetime, :to => YasuiForm::Inputs::DateTimeInput
    map_type :country, :time_zone, :to => YasuiForm::Inputs::PriorityInput
    map_type :boolean, :to => YasuiForm::Inputs::BooleanInput
    ...
    # by default.
    #
    # The fact YasuiForm is built in components allow the interface to be unified.
    ...
    # given YasuiForm.time_zone_priority and YasuiForm.country_priority are used respectivelly.
    #
    def input(attribute_name, options={}, &block)
      ...
      if block_given?
        YasuiForm::Inputs::BlockInput.new(self, attribute_name, column, input_type, options, &block).render
      else
    ...
    # == Block
    #
    # When a block is given, association yasui behaves as a proxy to
    # yasui_fields_for:
    #
    ...
    def association(association, options={}, &block)
      return yasui_fields_for(*[association,
    ...
    def error(attribute_name, options={})
      ...
      column = find_attribute_column(attribute_name)
      input_type = default_input_type(attribute_name, column, options)
      YasuiForm::Inputs::Base.new(self, attribute_name, column, input_type, options).error
    end
    ...
    def hint(attribute_name, options={})
      ...
      YasuiForm::Inputs::Base.new(self, attribute_name, column, input_type, options).hint
    end
    ...
    def label(attribute_name, *args)
      ...
      column = find_attribute_column(attribute_name)
      input_type = default_input_type(attribute_name, column, options)
      YasuiForm::Inputs::Base.new(self, attribute_name, column, input_type, options).label
    end
    ...
    def error_notification(options={})
      YasuiForm::ErrorNotification.new(self, options).render
    end
  ...
  private
    ...
    def find_custom_type(attribute_name) #:nodoc:
      YasuiForm.input_mappings.find { |match, type|
        attribute_name =~ match
      }.try(:last) if YasuiForm.input_mappings
    end

    def file_method?(attribute_name) #:nodoc:
      file = @object.send(attribute_name) if @object.respond_to?(attribute_name)
      file && YasuiForm.file_methods.any? { |m| file.respond_to?(m) }
    end
    ...
    # 3) If not, fallbacks to YasuiForm::Inputs::#{input_type}Input
    ...
    def mapping_override(klass) #:nodoc:
      name = klass.name
      if name =~ /^YasuiForm::Inputs/
        attempt_mapping name.split("::").last, Object
      end
    end

    def attempt_mapping(mapping, at) #:nodoc:
      return if YasuiForm.inputs_discovery == false && at == Object
